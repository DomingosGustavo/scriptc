...
static const char web_prelude[] =
    ...
    "      if (typeof value.name === 'string') return new File([value], value.name, { type: value.type || '' });\n"
    "      return value;\n"
    "    }\n"
    "    return String(value);\n"
    "  };\n"
    ...
    "          throw new TypeError(\"Failed to construct 'FormData': parameter 1 is not of type 'object'\");\n"
    "        }\n"
    "        if (typeof form[Symbol.iterator] === 'function') {\n"
    "          for (const pair of form) {\n"
    "            if (typeof pair !== 'object' || pair === null) {\n"
    "              throw new TypeError(\"Failed to construct 'FormData': iterator expected [name, value] pairs\");\n"
    "            }\n"
    "            this.append(pair[0], pair[1], pair[2]);\n"
    "          }\n"
    "        }\n"
    "      }\n"
    "    }\n"
    ...
    "    get(name) {\n"
    "      const n = String(name);\n"
    "      for (const e of this._entries) { if (e[0] === n) return e[1]; }\n"
    "      return null;\n"
    "    }\n"
    "    getAll(name) {\n"
    "      const n = String(name);\n"
    "      const out = [];\n"
    "      for (const e of this._entries) { if (e[0] === n) out.push(e[1]); }\n"
    "      return out;\n"
    "    }\n"
    "    has(name) {\n"
    "      const n = String(name);\n"
    "      for (const e of this._entries) { if (e[0] === n) return true; }\n"
    "      return false;\n"
    "    }\n"
    "    set(name, value, filename) {\n"
    "      const n = String(name);\n"
    "      const entry = [n, formDataValue(value, filename)];\n"
    "      let replaced = false;\n"
    "      const out = [];\n"
    "      for (const e of this._entries) {\n"
    "        if (e[0] === n) { if (!replaced) { out.push(entry); replaced = true; } continue; }\n"
    "        out.push(e);\n"
    "      }\n"
